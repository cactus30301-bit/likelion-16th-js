...
return resultLog + ' ' + numberNth + '기 여러분! ' + '중도 이탈은 안돼요!!🥲'
// 종료 이후엔 코드 실행이 안됨
...
//명시적으로 결과 값 반환
return '멋사 프론트엔드 부트캠프 ' + parseInt(nth, 10) + '기 ' + fightingMessage
...
console.log(logger(14, '멋진 개발자 되세요'))
...
// 아래는 그냥 참고 하는 용도로만 보기 (예전 방식)
// 함수 선언 또는 함수 표현식 (표 생성하는 함수(기능))
// JavaScript를 사용해 마크업(구조설계) > html로 원래 구조설계를 하지만 JS로도 할 수 있다.
// 표 생성하는 함수(기능)
...
// * 일시적 사각지대(TDZ) : 변수 선언(생성) 후, 초기화 전까지 접근 불가능한 구간입니다. > 안전 펜스
...
// 블록(Blcok, {...}) - 지역 생성
{
  // 블록 내부의 범위(Scopt)에서만 사용 가능한 지역변수 (local variable)
  // let, const 키워드로 정의된 변수는 외부에서 접근 (외부에서 접근 불가능)
  // 하지만, var 키워드로 정의된 변수는 지역 만들지 않음 (외부에서 접근 가능)
...
// var 변수로 정의된 것은 호이스팅 (끌어올리는 것처럼 보여요)
// var 변수의 초기화가 없다면? JS엔진이 undefined로 초기 값을 설정
console.log(notTdz)

// tdz (초기화 이전 상태ㅣ <Uninitialized>)
// console.log(tdz)


let tdz // = '✅let, const 변수는 "일시적 사각 지대(TDZ)--안전펜스" 생성'
